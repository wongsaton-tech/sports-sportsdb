...
// เซ็ตค่าสแตนด์บายเริ่มต้นสำหรับบอร์ดคะแนน เพื่อไม่ให้ระบบพังไม่ว่าตารางอื่นจะมีคอลัมน์อะไร
$teams_dashboard = [];
$has_scores = false;

try {
    // ------------------------------------------------------------------
    // ส่วนที่ 1: ดึงรายชื่อทีมสีแบบปลอดภัยที่สุด (ดึงเฉพาะคอลัมน์พื้นฐานที่มีแน่นอน)
    // ------------------------------------------------------------------
...
try {
    // ------------------------------------------------------------------
    // ส่วนที่ 2: ดึงข้อมูลปฏิทินรายการแข่งขัน (คอนเฟิร์มชื่อตาม DBeaver)
    // ------------------------------------------------------------------
    $sql_matches = "SELECT m.* FROM matches m ORDER BY m.date_time ASC";
    ...
    foreach ($all_matches as $match) {
        ...
        // ตรวจสอบเงื่อนไขเวลาปัจจุบันกับเวลาแข่งขัน
        if (!empty($match_time)) {
    ...
    $calendar_timeline = $upcoming_list;

} catch (\PDOException $e) {
    die("<div class='container mt-5 alert alert-danger text-center'>เกิดข้อผิดพลาดในการเชื่อมโยงตารางแมตช์: " . $e->getMessage() . "</div>");
}
