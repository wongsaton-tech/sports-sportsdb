...
// ------------------------------------------------------------------
// ส่วนที่ 1: ดึงเฉพาะรายชื่อทีมสีแบบปลอดภัยที่สุด (ตัดการ Join ตารางคะแนนที่ล่มออกไป)
// ------------------------------------------------------------------
$teams_dashboard = [];
$has_scores = false;

try {
    // ดึงเฉพาะ id และ team_name จากตาราง teams ที่คอนเฟิร์มว่ามีแน่นอนใน DBeaver
    ...
    // ป้องกันกรณีฉุกเฉิน ไม่ให้ระบบหยุดทำงาน
}

// ------------------------------------------------------------------
// ส่วนที่ 2: ดึงข้อมูลปฏิทินรายการแข่งขัน (อิงตามชื่อคอลัมน์ใน DBeaver ของคุณครู)
// ------------------------------------------------------------------
$calendar_timeline = [];

try {
    // ดึงเฉพาะตาราง matches โดด ๆ ไม่ Join ตารางอื่นเพื่อความปลอดภัยสูงสุด
    ...
    foreach ($all_matches as $match) {
        ...
        if (!empty($match_time)) {
            ...
            if ($time_diff < 0) {
                // เกินเวลาเริ่มมาแล้ว สมมติสถานะกำลังแข่งขัน (ภายใน 2 ชั่วโมง)
    ...
    // จัดเรียงลำดับให้รายการที่กำลังจะแข่งเร็วที่สุดขึ้นก่อน
    ...
    // หากตาราง matches มีปัญหา ให้สร้างอาเรย์ว่างระบบจะได้ไม่ล่ม
    $calendar_timeline = [];
}

// ฟังก์ชันสุ่มเฉดสีระบบอัตโนมัติประจำทีมสไตล์ iOS คลีน ๆ ป้องกันการดึงคอลัมน์สีล่ม
...
// ตรวจสอบสถานะสิทธิ์ผู้ใช้งานจาก Session
